...
```bash
# yarn
yarn create astro@latest -- --template jdrhyne/volks-typo
...
### Testing
Run the comprehensive Playwright test suite with `yarn test:features`. Tests cover dark mode functionality, MDX rendering, reading time display, TOC navigation, and visual regression testing.
...
| `yarn astro ...` | Run CLI commands like `astro add` |
| `yarn check` | Check TypeScript types |
| `yarn lint` | Run ESLint |
| `yarn test:features` | Run Playwright feature tests |
...
SITE=https://example.com BASE_PATH=/blog/ yarn build
```
